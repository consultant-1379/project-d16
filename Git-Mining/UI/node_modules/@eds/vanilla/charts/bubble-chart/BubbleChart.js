...
export class BubbleChart {
  ...
  constructor(props) {
    ...
    this.r = { unit: '', tickFormat: '', min: 14, max: 64, ...props.r };
    ...
    // Default dot diameter 14x14, same as icon in legend pill. Maximum dot diameter 64x64
    this.diameter = { min: 14, max: 64, ...props.diameter};
    this.radiusMin = this.diameter.min / 2;
    this.radiusMinHover = Math.max(this.radiusMin, 7);
    this.radiusMax = this.diameter.max / 2;
    // this.radiusMax = +(this.height * 0.1).toFixed(); - alternative
  ...
  init() {
    ...
    this.canvas.classed('bubble-chart', true);
    ...
    // Fix domain so colors order stays correct when values are sorted
    this.colorScale.colorScale.domain(this.data.series.map(s => s.name));
  ...
  draw(data) {
    ...
    // Order by size, so smaller circle whould be above large
    this.values = this._getValues().sort((a,b) => b.r - a.r);

    this.width = this.stage.getCanvasWidth();

    this._createX();
    this._createY();
    this._createSizeLegend();
  ...
  _createX() {
     ...
     */
    const xMinRange = 0;
    const xMaxRange = this.width;

    // Create temporary scale with extended range
    const xScaleTemp = this.x.isLogarithmic ? d3.scaleLog() : d3.scaleLinear();
    xScaleTemp
      .domain([xMin, xMax])
      .range([xMinRange + this.radiusMax, xMaxRange - this.radiusMax]);
    ...
    // Create the scale with extended domain
  ...
  _createY() {
     ...
     * https://stackoverflow.com/questions/20960797
     */
    const yMinRange = this.height;
    const yMaxRange = 0;

    // Create temporary scale with extended range
    const yScaleTemp = this.y.isLogarithmic ? d3.scaleLog() : d3.scaleLinear();
    yScaleTemp
      .domain([yMin, yMax])
      .range([yMinRange - this.radiusMax, yMaxRange + this.radiusMax]);
   ...
   */
  _createSizeLegend() {
    const min = d3.min(this.values, d => d.r);
    const max = d3.max(this.values, d => d.r);

    const sizeLegend = this.canvas.select('.size-legend').empty() ?
      this.canvas.append('g').classed('size-legend', true) :
      this.canvas.select('.size-legend');

    // Same as Y label position
    sizeLegend.attr('transform', `translate(${this.width}, -${this.fontSize + this.axisPadding})`);

    sizeLegend.select('text').remove();

    const text = sizeLegend.append('text').attr('text-anchor', 'end');

    const icon = text.append('tspan')
      .attr('class', 'icon icon-sizes')
      .attr('font-size', '16px');

    // Use content value of ':before' because icon code can change
    icon.text((getComputedStyle(icon.nodes()[0], ':before').content).slice(1, -1));
    ...
    text.append('tspan')
      .attr('dx', '16')
      .text(`Min: ${min ? d3.format(this.r.tickFormat)(min) : '-'}`);

    text.append('tspan')
      .attr('dx', '16')
      .text(`Max: ${max ? d3.format(this.r.tickFormat)(max) : '-'}`);
  }
  ...
  _drawDots() {
    const rScale = d3.scaleSqrt()
      .domain(d3.extent(this.values.map(d => d.r)))
    ...
    dotGroupsEnter
      ...
      .attr('cx', d => this.xScale(d.x))
      .attr('cy', d => this.yScale(d.y))
      .attr('r', d => Math.max(this.radiusMinHover, rScale(d.r)))
      .attr('fill', 'blue')
      .attr('opacity', 0)
